...
const getNftByContractAddress = (address, tokenId) => {
    const options = {
        headers: {
    ...
    return axios_1.default.post(process.env.API_MONODERAFL_GRAPHQLAPIENDPOINTOUTPUT, body, options);
};
const getCollectionByContractAddress = (address) => {
    const options = {
        headers: {
            'x-api-key': process.env.API_MONODERAFL_GRAPHQLAPIKEYOUTPUT
        }
