...
export default function CollectionUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setContractDeployer(cleanValues.contractDeployer);
    setDeployedBlockNumber(cleanValues.deployedBlockNumber);
    setTokenType(cleanValues.tokenType);
    setTotalSupply(cleanValues.totalSupply);
    setChainId(cleanValues.chainId);
    setName(cleanValues.name);
    setOpenseaSlug(cleanValues.openseaSlug);
    setImageUrl(cleanValues.imageUrl);
    setExternalUrl(cleanValues.externalUrl);
    setDiscordUrl(cleanValues.discordUrl);
    setTwitterUsername(cleanValues.twitterUsername);
    setDescription(cleanValues.description);
    setFloorPrice(cleanValues.floorPrice);
    setErrors({});
  };
  const [collectionRecord, setCollectionRecord] = React.useState(collection);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Collection, idProp)
        : collection;
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Collection.copyOf(collectionRecord, (updated) => {
