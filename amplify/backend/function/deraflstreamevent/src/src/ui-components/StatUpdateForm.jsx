...
export default function StatUpdateForm(props) {
  const {
    id: idProp,
    stat,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = statRecord
      ? { ...initialValues, ...statRecord }
      : initialValues;
    setType(cleanValues.type);
    setCreatedAt(cleanValues.createdAt);
    setUpdatedAt(cleanValues.updatedAt);
    setEthPaid(cleanValues.ethPaid);
    setTicketsBought(cleanValues.ticketsBought);
    setRoyaltiesPaid(cleanValues.royaltiesPaid);
    setChainId(cleanValues.chainId);
    setRafflesCreated(cleanValues.rafflesCreated);
    setErrors({});
  };
  const [statRecord, setStatRecord] = React.useState(stat);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Stat, idProp) : stat;
      setStatRecord(record);
    };
    queryData();
  }, [idProp, stat]);
  React.useEffect(resetStateValues, [statRecord]);
  const validations = {
    type: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Stat.copyOf(statRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "StatUpdateForm")}
      {...rest}
    >
      <TextField
        label="Type"
