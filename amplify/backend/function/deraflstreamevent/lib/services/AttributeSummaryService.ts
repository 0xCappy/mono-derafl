...
import { NFTAttribute, TraitWithPrice } from "../types"
...
export const mergeAttributeRarity = async (address: string, chainId: string, attributes: NFTAttribute[]) => {
    try {
        const summary = await getAttributeSummary(address, mapHexToAlchemyChain(chainId))
