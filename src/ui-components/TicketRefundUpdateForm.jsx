...
export default function TicketRefundUpdateForm(props) {
  const {
    id: idProp,
    ticketRefund,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = ticketRefundRecord
      ? { ...initialValues, ...ticketRefundRecord }
      : initialValues;
    setType(cleanValues.type);
    setCreatedAt(cleanValues.createdAt);
    setUpdatedAt(cleanValues.updatedAt);
    setRaffleNonce(cleanValues.raffleNonce);
    setEthAmount(cleanValues.ethAmount);
    setRefundee(cleanValues.refundee);
    setChainId(cleanValues.chainId);
    setErrors({});
  };
  const [ticketRefundRecord, setTicketRefundRecord] =
    React.useState(ticketRefund);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(TicketRefund, idProp)
        : ticketRefund;
      setTicketRefundRecord(record);
    };
    queryData();
  }, [idProp, ticketRefund]);
  React.useEffect(resetStateValues, [ticketRefundRecord]);
  const validations = {
    type: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            TicketRefund.copyOf(ticketRefundRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TicketRefundUpdateForm")}
      {...rest}
    >
      <TextField
        label="Type"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || ticketRefund)}
